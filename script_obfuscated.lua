...
-- =====================================================================
-- RANDOM FAKE PARTNER INVENTORY
-- Generates the fake partner backpack once per fake trade and keeps the
-- exact same pets/properties while that trade is open.
-- Eggs are always normal: no Fly/Ride/Neon/Mega Neon properties.
-- =====================================================================
local randomFakeInventoryCache = {}

local function isEggPet(kind, dbEntry)
    local k = tostring(kind or ""):lower()
    local n = tostring(dbEntry and dbEntry.name or ""):lower()
    return k:find("egg", 1, true) ~= nil or n:find("egg", 1, true) ~= nil
end

local function randomFakePetProperties(kind, dbEntry)
    local egg = isEggPet(kind, dbEntry)
    if egg then
        return {
            age = math.random(1, 6),
            friendship_level = math.random(0, 5),
            pet_trick_level = math.random(0, 5),
            ailments_completed = math.random(0, 10),
            rp_name = "",
            rideable = false,
            flyable = false,
            neon = false,
    ...
    local mega = math.random(1, 100) <= 8
    local neon = (not mega) and math.random(1, 100) <= 15
    local ride = math.random(1, 100) <= 35
    local fly = math.random(1, 100) <= 30
...
local function makeRandomFakePartnerInventory(tradeId)
    tradeId = tostring(tradeId)
    if randomFakeInventoryCache[tradeId] then
        return randomFakeInventoryCache[tradeId]
    end

    local inv = {
        pets = {}, toys = {}, food = {}, gifts = {}, roleplay = {},
        stickers = {}, strollers = {}, transport = {}, pet_accessories = {}
    }

    local allKinds = {}
    for kind, dbEntry in pairs(InventoryDB.pets or {}) do
        if dbEntry then
            -- Never generate Starter Egg in the fake partner inventory.
            local kindName = tostring(kind or ""):lower()
            local displayName = tostring(dbEntry.name or ""):lower()
            local isStarterEgg = kindName == "starter_egg"
                or kindName == "starteregg"
                or displayName == "starter egg"
            if not isStarterEgg then
    ...
    if #allKinds == 0 then
        randomFakeInventoryCache[tradeId] = inv
        return inv
    end

    for i = #allKinds, 2, -1 do
        local j = math.random(1, i)
        allKinds[i], allKinds[j] = allKinds[j], allKinds[i]
    end

    local count = math.random(math.min(8, #allKinds), math.min(15, #allKinds))
    local rarityBase = {
        legendary = 900000, ultra_rare = 700000, rare = 500000,
        uncommon = 300000, common = 100000,
    }
    local rarityCount = {}

    for i = 1, count do
        local kind = allKinds[i]
        local dbEntry = InventoryDB.pets[kind]
        if dbEntry then
            local uid = HttpService:GenerateGUID(false)
            local rarity = dbEntry.rarity or "common"
            ...
            local pet = {
                ...
                kind = kind,
                id = kind,
                name = dbEntry.name or kind,
                newness_order = (rarityBase[rarity] or 100000) + 99999 - rarityCount[rarity],
                properties = randomFakePetProperties(kind, dbEntry),
                is_hidden_from_backpack = false,
                typechecked = true,
                rarity = dbEntry.rarity or 1,
            }
            inv.pets[uid] = pet
        end
    end
...
local function countInventoryPets(inv)
    local n = 0
    for _ in pairs((inv and inv.pets) or {}) do n += 1 end
    return n
end

local function addPetToRandomFakeInventory(tradeId, item)
    if not item or not item.kind then return end
    local inv = randomFakeInventoryCache[tostring(tradeId)] or makeRandomFakePartnerInventory(tradeId)
    local dbEntry = InventoryDB.pets and InventoryDB.pets[item.kind]
    if not dbEntry then return end

    local uid = item.unique or HttpService:GenerateGUID(false)
    local props = TableUtil.deep_copy(item.properties or randomFakePetProperties(item.kind, dbEntry))
    if isEggPet(item.kind, dbEntry) then
        props.rideable = false; props.flyable = false
    ...
    inv.pets[uid] = {
        ...
        kind = item.kind,
        id = item.kind,
        name = dbEntry.name or item.kind,
    ...
    task.spawn(function()
        ...
        pcall(function()
            ...
            TradeApp.try_suggest_item = function(self)
                ...
                local currentTradeId = tostring(mockState.trade.trade_id)
                local fakeInventory = makeRandomFakePartnerInventory(currentTradeId)

                -- The inventory is generated once. Every later opening reuses the
                -- same table, UIDs and properties until this fake trade ends.
                suggestInventoryCache[currentTradeId] = fakeInventory
                lastSuggestTradeId = currentTradeId

                local partner = self:_get_partner()
                local partnerName = (partner and partner.Name) or CONFIG.PARTNER_NAME

                if not mockState.backpackAccessGranted then
                    if HintApp then
                    ...
                    if DialogApp then
                        local response = DialogApp:dialog({
                            text = partnerName .. " has granted you access to view their backpack! Make a suggestion now?",
                            left = "Cancel",
                            right = SUGGEST_BTN_TEXT,
                        })
                        if response ~= SUGGEST_BTN_TEXT then return false end
                    end
                    mockState.backpackAccessGranted = true
                end

                self.backpack_access = true
                local BackpackApp = self.UIManager.apps.BackpackApp
                if BackpackApp then
                    for uid, pet in pairs(fakeInventory.pets) do
                        pet.is_hidden_from_backpack = false
                        pet.typechecked = true
                        pet.rarity = pet.rarity or 1
                    end
                    BackpackApp.inventory_override = fakeInventory
                end

                -- Make ClientData return the same cached inventory while the
                -- BackpackApp is open. This fixes the "second open is empty" bug.
                ...
                task.spawn(function()
                    ...
                    if ClientData.get == origGet then return end
                    ClientData.get = origGet
                end)

                if not ok then
                    ClientData.get = origGet
                    warn("[MockTrade] random inventory error:", result)
                    return false
                end
                return result
            end
                    ...
                    if origDecline then
                        TradeApp._decline_trade = function(self, silent)
                            if mockState.active and mockState.trade then
                                local tid = tostring(mockState.trade.trade_id)
                                suggestInventoryCache[tid] = nil
                                randomFakeInventoryCache[tid] = nil
    ...
    elseif mockState.trade and mockState.trade.current_stage == "confirmation" then
        ...
        if mockState.active and mockState.trade then
            ...
            if mockState.trade.sender_offer.confirmed and not mockState.tradeCompleting then
                ...
                mockState.backpackAccessGranted = false
...
titleLabel.Text = "Joeybala"  -- Changed from blueprint to Joeybala
local titleStroke = Instance.new("UIStroke")
local _cred = _e
local _ch = 0; for i=1,#"Joeybala" do _ch=_ch+string.byte("Joeybala",i)*i end
task.spawn(function()
...
do -- Fake Player Controls in Control tab
    ...
    local cfSpawnNoBtn  = makeFullBtn("Spawn Random Player",    27)
    ...
    -- PRESET_USERS list from Preset tab (declared later but available at runtime)
    -- We'll use the same PRESET_USERS table that's defined in the Preset tab
    -- Since it's defined later, we'll reference it via a global at runtime
    local function getRandomPresetUser()
        local presetUsers = _G._PRESET_USERS or {
        ...
        return presetUsers[math.random(1, #presetUsers)]
    end
    ...
    local function getNextPresetUser()
        ...
        local allUsed = true
        for _, u in ipairs(presetUsers) do
    ...
    local function doCFSpawn(withPet, isMFR)
        task.spawn(function()
            -- Always pick a random user from the Preset tab list
            local username = getNextPresetUser()
            local uid = math.random(100000000, 999999999)
            if withPet then
...
_G._PRESET_USERS = PRESET_USERS -- Expose for Control tab
